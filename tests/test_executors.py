...
def test_replay_frames_doc_godot_schema(tmp_path):
    """The godot substrate obeys the SAME {meta, frames} contract as js/py:
    meta carries title/prompt/world_size + engine=='godot'; every frame is
    {tick, entities} with monotone ticks starting at 0 and non-empty entities;
    floats are rounded to <= 2 decimals and the payload gzips small.
