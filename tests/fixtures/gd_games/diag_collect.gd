# diag_collect.gd -- DIAGONAL-advantage collect fixture (CHORD Phase-2 bench; see CHORD_PIVOT.md §8).
#
...
func build(world_seed: int) -> void:
	...
	_gems = []
	_add_gem("gem_a", Vector2(450.0, 150.0))                 # (+150,-150): up+right diagonal
	_add_gem("gem_b", Vector2(150.0, 450.0))                 # (-150,+150): down+left diagonal
