...
(``actions[i]``), and the reward (``step_reward``: bounded checkpoint shaping + per-tick
living cost + time-decayed terminal) are byte-for-byte those of ``GodotServeEnv.step`` — so the
    ...
    class _GodotBatchVecEnv(VecEnv):
        ...
        def step_wait(self):
            ...
            for i in range(n):
                ...
                # PlanckEnv): bounded shaping + per-tick living cost + time-decayed terminal.
