"""harness CLI — the `game` group (new / verify / replay / capture / export /
attack / rescue / harden / breed / demo) plus the rl / bank / ledger groups.
...
def _game_witness(game_path: str) -> dict:
    ...
    a fresh verify (engine-agnostic: verify_game routes godot/gdscript internally)."""
...
def cmd_game_replay(args) -> int:
    ...
    Dispatches by ENGINE (godot -> render_godot_replay, driving a `.spec.json`;
    gdscript -> render_gdscript_replay, driving a `.gd` game through the serve
    host). ``--frames PATH`` additionally persists the replay substrate
    ({meta, frames}). When ``--frames`` is given without an explicit ``--gif``,
    only the substrate is written."""
        ...
        if engine in ("godot", "gdscript"):
            try:
                from harness.verify.executors import (
                    render_gdscript_replay, render_godot_replay,
                )
            except Exception as exc:  # noqa: BLE001
                return _module_missing("executors", exc, args.json)
            render_fn = {"godot": render_godot_replay,
            ...
            return _call_error(
                "game replay",
                ValueError(f"unsupported engine {engine!r}: only godot / gdscript "
                           f"games can be replayed"),
                args.json)
        out["gif"] = gif_res
...
def cmd_rl_probe(args) -> int:
    ...
    RL witness replays via the batch executor; emits ONE JSON line (ledger-friendly)
    whose keys are g3_prime's own — learnable, stochastic_success_rate, bridge_ok, ..."""
    try:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    rp.add_argument("--chord", action="store_true",
