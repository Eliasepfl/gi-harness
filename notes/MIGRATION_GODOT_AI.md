...
Status: **PoC PASSED — feasibility proven.** Integration lane assessment + plan.
...
- **Layering recommendation: adapter, not host change.** Keep the entire certification
  ...
  `no load()/preload()` ban.
- **Hermes Agent is optional.** It is one of ~20 interchangeable MCP clients. We drive
  `http://127.0.0.1:8000/mcp` from our own orchestration. Skip Hermes.
...
**What it is:** NousResearch's **Hermes Agent** (`github.com/NousResearch/hermes-agent`) — an
open-source, model-agnostic *agentic CLI/client* that speaks MCP; config lives at
`~/.hermes/config.yaml`, and its `hermes mcp add [--preset]` manages MCP servers (including
remote/OAuth 2.1 endpoints). `godot-ai` lists it among
~20 auto-configurable clients (Claude Code, Claude Desktop, Codex, Grok Build, Cursor, Antigravity,
Cline, …). "Auto-configure" simply writes the `http://127.0.0.1:8000/mcp` endpoint into each
client's own config format.

**Is it required?** No. `godot-ai` is a standard FastMCP **streamable-http** server. *Any* MCP
client can drive it — including a **custom Python client** (the PoC used `fastmcp.Client`), Claude
Code, or an **Anthropic-API agent loop** pointed at `http://127.0.0.1:8000/mcp`. Hermes is one
interchangeable option, differentiated only by running a local open-weights model (offline/cost).

**Recommendation for us:** **drive `godot-ai` from our own orchestration** (the generation agent
that already runs the pipeline), not Hermes. We want the *strong* driver (our best model + the
harness's contract + repair directives) in the loop, tight control over per-game session lifecycle
on the compute node, and no extra moving part. Hermes offers nothing we need; its local-model angle
is a downgrade from our orchestration. Keep it out of the critical path.
...
- Add a **`godot-ai` authoring backend** alongside the current blind-LLM backend in
  ...
  - **session launcher** — sbatch recipe from the PoC (editor in sif under Xvfb + uv server on
    host, per compute node) + per-game project scaffolding + teardown.
  - **MCP driver** — our agent loop over `http://127.0.0.1:8000/mcp` (fastmcp `Client`), handed
    `api_gdscript.md` as the compile target; authors via `script_create`/`script_patch`,
    plays via `project_run`/`game_manage`, observes via `editor_screenshot`/`logs_read`/`test_run`.
  - **extract-and-verify adapter** (§3) — pull the script source, run the *unchanged* `verify_game`.
  - **repair wiring** — feed `feedback.py`/`harden.py` directives back as MCP revisions.
- Keep the blind-LLM backend for breadth/cost; select per game (`backend="godotai"` vs today's
  `"auto"`).

### Phases
- **Phase 0 — PoC (DONE).** Editor+plugin+server+one tool call on a compute node. `18183129` PASS.
- **Phase 1 — Session harness.** Productionize the launcher: warm editor per node, deterministic
  ...
  `harness ... godotai-session` smoke that boots an editor and returns a live MCP handle.
- **Phase 2 — Author-and-verify one game.** Drive the agent to write one GameAPI `.gd` live
  (with `project_run`/`test_run` feedback), extract it, and pass the **full existing** funnel to
  CERTIFIED. Compare quality/structure against a blind-LLM baseline on the same prompt.
- **Phase 3 — Loop + repair.** Wire `feedback.py`/`harden.py` directives back into the editor
  session; measure repair convergence vs. blind regen.
- **Phase 4 — Throughput + routing.** Measure editor boot + per-game wall time; decide the split
  (godot-ai for high-value/hard prompts, blind-LLM for breadth). Batch across nodes. Wire into the
  wave runner.
- **Phase 5 — (optional) A1 flattener** as a spatial scratchpad, only if it demonstrably helps.
